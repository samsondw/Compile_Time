...
#define GET_MONTH(str, i) (str[i] == 'J' && str[i + 1] == 'a' && str[i + 2] == 'n' ? 1 : str[i] == 'F' && str[i + 1] == 'e' && str[i + 2] == 'b' ? 2 : str[i] == 'M' && str[i + 1] == 'a' && str[i + 2] == 'r' ? 3 : str[i] == 'A' && str[i + 1] == 'p' && str[i + 2] == 'r' ? 4 : str[i] == 'M' && str[i + 1] == 'a' && str[i + 2] == 'y' ? 5 : str[i] == 'J' && str[i + 1] == 'u' && str[i + 2] == 'n' ? 6 : str[i] == 'J' && str[i + 1] == 'u' && str[i + 2] == 'l' ? 7 : str[i] == 'A' && str[i + 1] == 'u' && str[i + 2] == 'g' ? 8 : str[i] == 'S' && str[i + 1] == 'e' && str[i + 2] == 'p' ? 9 : str[i] == 'O' && str[i + 1] == 'c' && str[i + 2] == 't' ? 10 : str[i] == 'N' && str[i + 1] == 'o' && str[i + 2] == 'v' ? 11 : str[i] == 'D' && str[i + 1] == 'e' && str[i + 2] == 'c' ? 12 : 0)
...
#define _UNIX_TIMESTAMP_YDAY(year, month, day) \
    (                                          \
        /* January */ day /* February */ + (month >= 2 ? 31UL : 0UL) /* March */ + (month >= 3 ? _UNIX_TIMESTAMP_FDAY(year) : 0UL) /* April */ + (month >= 4 ? 31UL : 0UL) /* May */ + (month >= 5 ? 30UL : 0UL) /* June */ + (month >= 6 ? 31UL : 0UL) /* July */ + (month >= 7 ? 30UL : 0UL) /* August */ + (month >= 8 ? 31UL : 0UL) /* September */ + (month >= 9 ? 31UL : 0UL) /* October */ + (month >= 10 ? 30UL : 0UL) /* November */ + (month >= 11 ? 31UL : 0UL) /* December */ + (month >= 12 ? 30UL : 0UL))

// get the UNIX timestamp from a digits representation
#define _UNIX_TIMESTAMP(year, month, day, hour, minute, second) \
    (/* time */ second + minute * SEC_PER_MIN + hour * SEC_PER_HOUR + /* year day (month + day) */ (_UNIX_TIMESTAMP_YDAY(year, month, day) - 1) * SEC_PER_DAY + /* year */ (year - 1970UL) * SEC_PER_YEAR + ((year - 1969UL) / 4UL) * SEC_PER_DAY - ((year - 1901UL) / 100UL) * SEC_PER_DAY + ((year - 1601UL) / 400UL) * SEC_PER_DAY)
